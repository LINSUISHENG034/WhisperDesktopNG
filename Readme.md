...
**WhisperDesktopNG** now includes full support for GGML quantized models through successful integration of whisper.cpp 1.7.6! This major enhancement brings:
...
2. **Phased implementation**: UTF-8 support → Pure C compilation → AVX2 optimization → x86 quantization files
...
The GGML quantized model support is implemented as a separate static library (`GGML.lib`) with the following key components:

- **Core GGML files**: `ggml.c`, `ggml-alloc.c`, `ggml-cpu.c`
- **x86 optimizations**: `x86-quants.c`, `x86-cpu-feats.cpp`, `x86-repack.cpp`
- **Whisper integration**: `whisper.cpp` compiled with C++ compatibility
- **Build configuration**: Pure C compilation with AVX2 support and UTF-8 encoding
...
For detailed technical documentation, see `Docs/implementation/GGML_Integration_Complete_Report.md`.
