/*
 * test_engine_new.cpp - Test cases for CWhisperEngineNew using latest whisper.cpp API
...
// Include our new CWhisperEngine header
#include "../Whisper/CWhisperEngineNew.h"
...
void testBasicFunctionality(const std::string& modelPath) {
    ...
    try {
        ...
        CWhisperEngineNew engine(modelPath, config);
...
void testAdvancedFeatures(const std::string& modelPath) {
    ...
    try {
        CWhisperEngineNew engine(modelPath);
        ...
        CWhisperEngineNew engine2 = std::move(engine);
...
void testQuantizedModelSupport(const std::string& modelPath) {
    ...
    try {
        ...
        try {
            CWhisperEngineNew gpuEngine(modelPath, gpuConfig);
        ...
        CWhisperEngineNew cpuEngine(modelPath, cpuConfig);
        auto testAudio = generateTestAudio(16000, 2.0f);
        auto start = std::chrono::high_resolution_clock::now();
...
int main() {
    std::cout << "=== CWhisperEngineNew Test Suite (Latest whisper.cpp API) ===" << std::endl;
    ...
    try {
        ...
        std::cout << "CWhisperEngineNew with latest whisper.cpp API is working correctly!" << std::endl;
