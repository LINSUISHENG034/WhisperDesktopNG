/*
 * CWhisperEngineNew.cpp - Implementation using latest whisper.cpp API
 */

#include "stdafx.h"
#include "CWhisperEngineNew.h"
...
CWhisperEngineNew::CWhisperEngineNew(const std::string& modelPath, const TranscriptionConfig& config)
...
CWhisperEngineNew::~CWhisperEngineNew() {
    if (m_ctx) {
        whisper_free(m_ctx);
        m_ctx = nullptr;
    }
...
CWhisperEngineNew::CWhisperEngineNew(CWhisperEngineNew&& other) noexcept
    : m_ctx(other.m_ctx), m_defaultConfig(std::move(other.m_defaultConfig)) {
    other.m_ctx = nullptr;
}

CWhisperEngineNew& CWhisperEngineNew::operator=(CWhisperEngineNew&& other) noexcept {
    if (this != &other) {
        if (m_ctx) {
            whisper_free(m_ctx);
        }
...
TranscriptionResult CWhisperEngineNew::transcribe(const std::vector<float>& audioData) {
...
whisper_full_params CWhisperEngineNew::createWhisperParams(const TranscriptionConfig& config) const {
...
TranscriptionResult CWhisperEngineNew::extractResults() const {
...
void CWhisperEngineNew::validateAudioData(const std::vector<float>& audioData) const {
    if (audioData.empty()) {
...
std::string CWhisperEngineNew::getModelType() const {
    if (!m_ctx) {
        throw CWhisperError("Whisper context is not initialized.");
    }
...
bool CWhisperEngineNew::isMultilingual() const {
    if (!m_ctx) {
        throw CWhisperError("Whisper context is not initialized.");
    }
...
std::vector<std::string> CWhisperEngineNew::getAvailableLanguages() const {
...
void CWhisperEngineNew::resetTimings() {
    if (m_ctx) {
        whisper_reset_timings(m_ctx);
    }
}

void CWhisperEngineNew::printTimings() const {
    if (m_ctx) {
